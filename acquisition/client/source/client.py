...
class ClientController(threading.Thread):
    ...
    def startRecording(self, _seg_size: int, streamPort: int, serverTime: float) -> None:
        ...
        if not self.recording:
            ...
            if (self.previewing and self.camera.resolution == (1280, 720) and self.camera.framerate > 30.0) \
               or (self.previewing and self.camera.framerate > 60.0):
                self.stopPreviewing(serverTime)
                
            prevsize = (int(self.resolution[0]/(self.compression)),int(self.resolution[1]/(self.compression)))
...
#                         bitrate=BITRATE
#                 )
#             elif (self.camera.resolution == (640, 480) and self.camera.framerate > 60.0):
...
#                 self.server.error(self, 'This was called and should be called')                
...
#                     splitter_port=1,
#                 )
#                 self.server.error(self, 'called 1')
#             else:
...
#         self.server.error(self, 'self.camera.zoom: {}'.format(self.camera.zoom))
        ...
        if self.recording:
            if (not self.previewing) and self.camera.resolution == (1280, 720) and self.camera.framerate > 30.0 \
               or ((not self.previewing) and self.camera.framerate > 60.0):
                self.startPreviewing(self.previewPort, serverTime)
            try:
...
#             else:
#                 prevsize = (int(self.resolution[0]/(self.compression)),int(self.resolution[1]/(self.compression)))
            if self.camera.resolution == (640, 480) and self.camera.framerate == 60.0:
                self.camera.start_recording(
                        self.preview.stdin,
                        format='h264',
                        resize=(320, 240),
            ...
            elif self.camera.resolution == (1280, 720) and self.camera.framerate <= 30:
                self.camera.start_recording(
                        self.preview.stdin,
                        format='h264',
                        #resize=(320, 180),
    ...
    def stopPreviewing(self, serverTime: float=0) -> None:
        ...
        """
        #return
        if self.previewing or self.fpsNeeded:
            splitNotificationTime = time.time()
            offset = splitNotificationTime - serverTime
...
#             if self.camera.framerate >= 60.0:
#                 self.camera.sensor_mode = 4
#                 
#             else:
#                 self.camera.sensor_mode = 4
#         self.server.error(self, 'sensor mode set is {}'.format(self.camera.sensor_mode))
...
#         self.checkZoom()
    ...
    def setFPS(self, fps: int) -> None:
        ...
        try:
            ...
            self.server.error(
                    self, 'Unable to set frame rate of camera to {}: {} now fixing'.format(
                            fps, err))
            try:
                self.stopPreviewing()
                self.camera.framerate = fractions.Fraction(fps, 1)
                self.server.error(
                        self, 'Was able to fix, set frame rate of camera to {}: {} now fixing'.format(
                                fps, err))
            except Exception as err:
                self.server.error(
